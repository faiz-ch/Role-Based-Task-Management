...
export async function updateUser(id: number, data: { name?: string; email?: string; password?: string; active?: boolean }): Promise<UserType> {
  const payload: any = {};
...
export async function createUser(data: { name: string; email: string; password: string; role_id?: number }): Promise<UserType> {
